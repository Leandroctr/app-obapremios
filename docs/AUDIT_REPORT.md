...
- **M-9 — corrigido:** removida a policy anon `Allow public read app_settings`
  ...
  `source: "database"` e tenant `pwa.app-obapremios.com`.
- **C-2 — corrigido na branch:** `app/page.tsx` passou de
  ...
  same-origin. O tenant OBA usa atualmente splash estática, portanto o caminho
  ativo não sofreu alteração. TypeScript e build passaram; lint passou sem
...
tenant OBA com o App ID correto e removido ao final.
...
settings reais do tenant OBA passou. A validação visual e o console de CSP em
...
Esta etapa foi executada somente no worktree OBA de segurança, a partir do HEAD
`b0ce8512e12ba80293a761203ffd9cc37c81b536`. Não houve migration, DDL, alteração
de configuração do bucket, commit, push ou deploy. As suítes reais criaram
...
real (incluindo o upload e o teardown), 7/7 A-5 estático e 13/13 A-5 real.
...
No OBA passaram: instalação reproduzível com `npm ci --no-audit --no-fund`,
